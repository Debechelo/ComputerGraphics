...
function incRotation() {
    rotations[cube] += 0.1;
    if (triangle_color[cube] > Math.PI)
        triangle_color[cube] = 0;
}

function decRotation() {
    rotations[cube] -= 0.1;
    if (triangle_color[cube] < 0)
        triangle_color[cube] = Math.PI;
}
...
function main() {
    ...
    void main(void) {
      gl_Position = uProjectionMatrix * uModelViewMatrix * uModelMatrix * aVertexPosition;
      vColor = aVertexColor;
    }
    `;
